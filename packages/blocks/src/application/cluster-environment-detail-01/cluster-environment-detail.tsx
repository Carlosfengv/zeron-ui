...
const defaultIncidents = [
  ...
  { id: "host-03", priority: "P1", resourceName: "PROD-HOST-03", resourceType: "HostVO", resourceKind: "host", title: "CPU Ready 持续高于阈值", description: "核心业务虚拟机调度延迟升高", duration: "持续26分钟", trend: "波动上升", related: "关联了 48 VM" },
  { id: "storage-02", priority: "P1", resourceName: "PS-生产存储-02", resourceType: "Ceph主存储", resourceKind: "storage", title: "副本恢复任务积压", description: "数据副本恢复速度低于预期", duration: "持续1小时", trend: "缓慢恢复中", related: "关联了 94 VM" },
...
function Incidents({ incidents, total, onSelect }: { incidents: readonly InspectionIncident[]; total: number; onSelect?: (incident: InspectionIncident) => void }) {
  ...
  const columns = useMemo<ColumnDef<InspectionIncident, unknown>[]>(
    ...
    [onSelect, priorityOptions, resourceTypeOptions],
  );
  ...
  return <section className="flex min-w-0 flex-col gap-2"><div className="flex items-center gap-2 px-2"><h2 className="text-body font-semibold text-fg-muted">异常事件</h2><Badge size="sm" status="info">{total}</Badge></div><DataTable className="[&_table]:min-w-[62rem]" emptyMessage="未找到符合条件的异常事件。" table={table}><DataTableToolbar showViewOptions={false} table={table} /></DataTable></section>;
}
