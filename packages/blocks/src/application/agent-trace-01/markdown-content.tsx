...
export function StreamMarkdown({ text, streaming }: { text: string; streaming: boolean }) {
  ...
  return <div className="min-w-0 max-w-full space-y-3 whitespace-pre-wrap break-words leading-6">{parts.map((part, index) => {
    if (part.startsWith("```")) {
      ...
      return <pre key={index} className="max-w-full overflow-x-auto rounded-md bg-surface-raised px-3 py-2 text-label leading-5"><code data-language={language || undefined}>{code}</code></pre>;
    }
