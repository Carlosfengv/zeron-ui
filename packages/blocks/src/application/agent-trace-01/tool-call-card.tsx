...
export function ToolCallCard({ item }: { item: ToolItem }) {
  ...
  return <section className="self-start w-full max-w-3xl rounded-md bg-surface-raised/60 px-3 py-2" data-tool-state={item.status}>
    <button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="flex w-full items-center gap-2 text-left outline-none focus-visible:ring-1 focus-visible:ring-focus-ring">
      ...
      <Badge size="sm" color={color}>{item.name}</Badge>
      <span className={cn("text-label", item.status === "running" && "shimmer-text text-fg-default", item.status === "error" && "text-fg-danger", item.status === "success" && "text-fg-muted")}>{state}</span>
      <span className="ml-auto text-label tabular-nums text-fg-subtle">{duration(item.durationMs)}</span>
    </button>
