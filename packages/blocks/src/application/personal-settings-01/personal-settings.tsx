...
function ProviderMark({ provider }: { provider: PersonalSettingsModelService["provider"] }) {
  ...
  return <span aria-hidden className="flex size-9 shrink-0 items-center justify-center rounded-lg border-hairline border-border bg-hover"><Icon size={24} /></span>;
}
...
function CredentialName({ credential }: { credential: PersonalSettingsCredential }) {
  return <div className="flex min-w-0 items-center gap-2"><span aria-hidden className="flex size-8 shrink-0 items-center justify-center rounded-lg border-hairline border-border bg-hover [&>svg]:block [&>svg]:size-5" dangerouslySetInnerHTML={{ __html: credentialBrandIcons[credential.brand].svg }} /><span className="truncate font-medium text-fg-default">{credential.name}</span></div>;
}
...
export function PersonalSettings({ account, actions, brand, className, data, defaultView = "keys", enabledViews, labels, loading, localeOptions, lockedNavigation = false, onViewChange, operationState, view: controlledView, ...props }: PersonalSettingsProps) {
  ...
  return (
    <AppShell layout="stacked" className={cn("@container h-full min-h-[46rem] overflow-hidden rounded-xl border-hairline border-border bg-surface-base", className)} {...props}>
...
function AccountInfoItem({ action, description, destructive = false, disabled = false, grouped = false, onAction, title }: { action: string; description: string; destructive?: boolean; disabled?: boolean; grouped?: boolean; onAction?: () => void; title: string }) {
  return <InfoItem className={cn("min-h-[76px] px-4 py-3.5", !grouped && "rounded-lg border-hairline border-border")}><InfoItemContent><InfoItemTitle>{title}</InfoItemTitle><InfoItemDescription>{description}</InfoItemDescription></InfoItemContent>{onAction && <InfoItemTrailing><Button disabled={disabled} onClick={onAction} size="md" type="button" variant={destructive ? "destructive" : "tertiary"}>{action}</Button></InfoItemTrailing>}</InfoItem>;
}
  ...
  return <div className="space-y-5 py-1">
    ...
    <Container>
      ...
      <ContainerBody className="p-3 sm:p-4">
        <section aria-labelledby="yearly-activity-title"><div className="overflow-x-auto pb-1"><div className="min-w-[720px]"><div className="grid grid-cols-12 gap-2">{data.months.map((month, monthIndex) => <div key={month}><p className="text-label text-fg-subtle">{month}</p><div className="mt-2 grid grid-flow-col grid-rows-7 gap-1">{Array.from({ length: 28 }, (_, index) => { const week = Math.floor(index / 7); const day = index % 7; const level = heatmapActivity[`${monthIndex}-${week}-${day}`] ?? 0; return <span aria-hidden className={cn("aspect-square rounded-xs", heatmapColor[level])} key={index} />; })}</div></div>)}</div></div></div><div className="mt-3 flex items-center justify-end gap-1.5 text-label text-fg-subtle"><span>较少</span>{heatmapColor.map((color, index) => <span aria-hidden className={cn("size-3 rounded-xs", color)} key={index} />)}<span>较多</span></div></section>
      </ContainerBody>
