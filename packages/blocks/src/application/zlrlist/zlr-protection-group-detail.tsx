  ...
  return (
    <section className="flex min-h-full min-w-0 flex-col rounded-lg border border-border-subtle p-3">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <InputGroup className="min-w-[180px] max-w-[450px] flex-1" size="md"><InputGroupAddon><Search aria-hidden="true" size={16} strokeWidth={1.5} /></InputGroupAddon><InputGroupInput aria-label="搜索虚拟机" onChange={(event) => nameColumn?.setFilterValue(event.target.value)} placeholder="搜索" value={(nameColumn?.getFilterValue() as string) ?? ""} /></InputGroup>
...
function OperationHistoryTab({ entries }: { entries: readonly ZlrOperationHistoryEntry[] }) {
  ...
  return (
    <section className="flex min-h-full min-w-0 flex-col rounded-lg border border-border-subtle p-3">
      <div className="mb-3"><InputGroup className="max-w-[450px]" size="md"><InputGroupAddon><Search aria-hidden="true" size={16} strokeWidth={1.5} /></InputGroupAddon><InputGroupInput aria-label="搜索操作历史" onChange={(event) => operationColumn?.setFilterValue(event.target.value)} placeholder="搜索" value={(operationColumn?.getFilterValue() as string) ?? ""} /></InputGroup></div>
                    ...
                    <div className="grid min-h-full min-w-0 gap-3 xl:grid-cols-[minmax(19rem,1fr)_minmax(0,2fr)]">
                      ...
                      <section className="flex min-h-0 min-w-0 flex-col rounded-lg border border-border-subtle p-3">
                        ...
                        <footer className="mt-1 flex min-h-9 flex-wrap items-center justify-end gap-3 px-3 py-1 text-label text-fg-muted">
                          <span>每页行数</span>
                          <Select itemDensity="compact" onValueChange={(value) => table.setPageSize(Number(value))} size="sm" value={`${table.getState().pagination.pageSize}`}><SelectTrigger aria-label="每页行数" className="w-[70px] min-w-[70px]" /><SelectContent>{[10, 20, 30, 40].map((size) => <SelectItem key={size} value={`${size}`}>{size}</SelectItem>)}</SelectContent></Select>
