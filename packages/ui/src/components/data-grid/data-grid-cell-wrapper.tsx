/* biome-ignore-all lint/a11y/useSemanticElements: Grid cells use div button semantics so selection, editing, and resize interactions share one focus model. */

"use client";

import * as React from "react";
import { useComposedRefs } from "#system/compose-refs";
import { getCellKey } from "#system/data-grid";
      ...
      className={cn(
        "size-full px-2 py-1.5 text-start text-body outline-none transition-colors duration-fast",
        {
          "ring-1 ring-inset ring-focus-ring":
            isFocused,
          "bg-selection":
            (isSearchMatch && !isActiveSearchMatch) || (isSelected && !isEditing),
