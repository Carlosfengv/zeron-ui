      ...
      <TableBody>
        {rows.map((issue, i) => (
          <TableRow key={issue.key} index={i}>
            ...
            <TableCell className="font-mono text-label text-fg-muted">
              {issue.key}
            </TableCell>
        ...
        return (
          <div key={col.key} className="flex flex-col gap-2.5">
            ...
            {items.map((issue) => (
              ...
              >
                <span className="text-body text-fg-default">{issue.title}</span>
    ...
    <div className={cn("flex flex-col gap-3 rounded-2xl p-4", surfaceClasses("raised", "raised"))}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span
            className="text-body text-fg-default font-semibold"
          >
            AI triage
          </span>
        </div>
  ...
  return (
    <ConceptFrame>
      <div className="mx-auto flex h-screen w-full max-w-[960px] flex-col gap-4 px-6 py-12">
        <Tabs value={String(view)} onValueChange={(value) => setView(Number(value))} variant="segment">
          {/* Toolbar */}
          <div className="flex flex-wrap items-center justify-between gap-3">
            <TabsList activationMode="manual">
              <TabItem value="0" label="Active" />
              <TabItem value="1" label="Backlog" />
              <TabItem value="2" label="Board" />
            </TabsList>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <InputFieldGroup className="w-56">
            <InputField
              index={0}
              label="Search"
              icon={Search}
