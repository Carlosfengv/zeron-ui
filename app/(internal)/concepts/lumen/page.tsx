...
function Answer() {
  return (
    <ChatMessage from="assistant" className="max-w-full">
      <div className="flex flex-col gap-3 text-[14px] leading-relaxed text-fg-default">
        <p>
      ...
      {SOURCES.map((s) => (
        ...
        >
          ...
          <div className="flex min-w-0 flex-col">
            <span
              className="truncate text-[13px] text-fg-default font-medium"
            >
              {s.title}
            </span>
...
function TurnView({ turn }: { turn: Turn }) {
  ...
  return (
    <article className="flex flex-col gap-4">
      <h2
        id="answer"
        className="text-[24px] leading-tight text-fg-default font-bold"
      >
        {turn.question}
      </h2>
      ...
      {!turn.thinking && (
        <Tabs value={String(tab)} onValueChange={(value) => setTab(Number(value))} variant="segment">
          <TabsList activationMode="manual">
            <TabItem value="0" icon={Sparkles} label="Answer" />
            <TabItem value="1" icon={Globe} label="Sources" />
            <TabItem value="2" icon={ImageIcon} label="Images" />
          </TabsList>
