  ...
  if (dismissed) {
    return (
      <button
        ...
        className="text-label text-fg-muted hover:text-fg-default underline underline-offset-4 cursor-pointer"
      >
        Restore card
      </button>
    );
...
function CardPlayground() {
  ...
  const controls = (
    <PlaygroundPanel onShuffle={randomize}>
      {/* Card (per-card props) */}
      <PlaySection label={t("card")} />
      <div>
        <PlayField label={t("media")}>
          <PlaySelect
            value={media}
      ...
      <PlaySection label={t("cardGroup")} />
      <div>
        <PlayField label={t("orientation")}>
          <PlaySelect
            value={orientation}
        ...
        <PlayField label={t("columns")} disabled={isInline}>
          <PlaySelect
...
export default function CardDoc() {
  ...
  return (
    <DocPage
      title="Card"
      slug="card"
      description="shadcn's compositional card, dressed in Zeron Design. Stacked, inline, or grid — borderless by default with proximity hover that previews where a click will land."
    >
      ...
      <DocSection title={t("outlinedGroup")}>
        <ComponentPreview code={outlinedCode} padding="compact">
          <OutlinedDemo />
        </ComponentPreview>
      </DocSection>

      <DocSection title={t("separatedTiles")}>
        <ComponentPreview code={separatedCode} padding="compact">
          <SeparatedDemo />
        </ComponentPreview>
      </DocSection>

      <DocSection title={t("fullyYours")}>
        <ComponentPreview code={promoCode} padding="compact">
          <PromoDemo />
        </ComponentPreview>
      </DocSection>

      <DocSection title={t("selected")}>
        <ComponentPreview code={selectedCode} padding="compact">
          <SelectedDemo />
        </ComponentPreview>
      </DocSection>

      <DocSection title={`${t("apiReference")} — Card`}>
        <PropsTable props={localize(cardProps, "c")} />
      </DocSection>

      <DocSection title={`${t("apiReference")} — CardGroup`}>
        <PropsTable props={localize(groupProps, "g")} />
      </DocSection>

      <DocSection title={`${t("apiReference")} — Parts`}>
        <PropsTable props={localize(partProps, "p")} />
      </DocSection>

      <DocSection title={`${t("apiReference")} — CardButton`}>
