...
const MERGE_TOKENS_PATH = `${ROOT}/packages/ui/src/system/tailwind-merge-tokens.ts`;
...
export function registryCssVars() {
  ...
  for (const token of motionDurationTokens) {
    theme[`transition-duration-${token.name}`] = `var(--motion-duration-${token.name})`;
  ...
  for (const token of borderWidthTokens) {
    theme[`border-width-${token.name}`] = token.value;
    light[`border-width-${token.name}`] = token.value;
    dark[`border-width-${token.name}`] = token.value;
  }
...
export function renderTailwindMergeTokensModule() {
  return `// Generated by scripts/generate-semantic-tokens.mjs. Do not edit.\n` +
    `export const mergeBorderWidthTokenNames = ${JSON.stringify(borderWidthTokens.map(({ name }) => name))} as const;\n` +
    `export const mergeMotionDurationTokenNames = ${JSON.stringify(motionDurationTokens.map(({ name }) => name))} as const;\n`;
}
...
function renderRootDeclarations() {
  ...
  for (const token of borderWidthTokens) lines.push(cssDeclaration(`border-width-${token.name}`, token.value));
...
export function renderDocumentation() {
  ...
  const borderWidthRows = borderWidthTokens.map((t) => [
    `\`--border-width-${t.name}\``, `\`border-${t.name}\``, t.value, t.usage,
  ]);
...
${table(["CSS 令牌", "Tailwind", "值", "用途"], borderWidthRows)}

\`hairline\` 用于细分隔线与轻量边界，并支持 \`border-t-hairline\`、\`border-x-hairline\` 等 Tailwind 方向变体。普通控件边框、强调边框和焦点指示器继续使用其既有宽度。
...
export function updateRegistry(registry) {
  ...
  item.description = "Complete semantic token system: color, typography, control sizes, border widths, motion durations, shadows, surfaces, and layers. Layout spacing and border radii use Tailwind's native scale. The historical surfaces slug is retained for compatibility.";
  ...
  const mergeTokensName = "tailwind-merge-tokens";
  const mergeTokensItem = registry.items.find((entry) => entry.name === mergeTokensName);
  const mergeTokensDefinition = {
    name: mergeTokensName,
    type: "registry:lib",
    title: "Tailwind Merge Tokens",
    description: "Generated semantic token names used by the Zeron class merger.",
    files: [
      {
        path: "packages/ui/src/system/tailwind-merge-tokens.ts",
        type: "registry:lib",
        target: "lib/tailwind-merge-tokens.ts",
      },
    ],
  };
  if (mergeTokensItem) Object.assign(mergeTokensItem, mergeTokensDefinition);
  else registry.items.splice(registry.items.indexOf(controlSizeItem ?? item) + 1, 0, mergeTokensDefinition);

  const utils = registry.items.find((entry) => entry.name === "utils");
  if (!utils) throw new Error('packages/ui/registry.json is missing the "utils" item');
  utils.registryDependencies = [
    ...new Set([...(utils.registryDependencies ?? []), mergeTokensName]),
  ];
...
async function expectedArtifacts() {
  ...
  return {
    ...
    mergeTokens: renderTailwindMergeTokensModule(),
  };
}
...
async function main() {
  ...
  const targets = [
    ...
    [MERGE_TOKENS_PATH, expected.mergeTokens],
  ];
  ...
  console.log("✓ generated packages/ui Tailwind merge token names");
}
