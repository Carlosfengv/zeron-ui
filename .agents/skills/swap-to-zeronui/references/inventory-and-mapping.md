...
- `status`: declared workflow state (`pending`, `migrating`, `partial`, `complete`). Follow [completion reconciliation](verification-and-recovery.md#reconcile-completion); the read-only checker never changes this field.
...
Keep region selection, template provenance/diffs, layout/scroll ownership and adapter exit records in migration-directory Markdown, keyed to existing mapping IDs. Do not add arbitrary fields to the closed plan schema. A zero count of old package imports does not establish cleanup: trace retained CSS and generic wrappers to their rendered responsibilities. Use exact text selectors for known residuals where appropriate, and human contract review for semantic duplication.

For each temporary generic adapter, record `export → callers → target public API → remaining difference → migration batch → exit evidence`. Include its actual project-relative implementation files, legacy import specifiers or specific residual text in mapping source selectors; selecting only the removed third-party package will miss its replacement bridge. Split mappings when verified callers and unfinished callers would otherwise share one verified label. Keep remaining work pending/migrated as appropriate, or blocked with strategy=gap for a missing capability; verify only after the mapped callers and states satisfy the contract. Do not select all files in a directory merely named `project-ui`: retained domain compositions are judged by responsibility. Follow [completion reconciliation](verification-and-recovery.md#reconcile-completion) before reporting success.

For charts, record the old renderer/files, chart purpose, series/query and aggregation semantics, required interactions, target Zeron Chart/Recharts composition, compatibility findings and verification. Follow the builder's [chart rules](../../zeron-page-builder/references/charts.md). Trace old wrappers and styles as well as packages. Do not select `recharts` as an old dependency to remove when the replacement still needs it; similarly, scope-external consumers may still need another renderer. Reference this inventory from existing mapping IDs and evidence rather than adding plan schema fields.

Each check kind permits one evidence record. A report's Markdown links are not recursively verified by `swap check`. For multi-file evidence, use the [evidence index procedure](verification-and-recovery.md#bind-reports-and-attachments); reference its index path/hash from existing check records and verify its listed files separately. The schema and canonical CLI schema asset remain unchanged.
