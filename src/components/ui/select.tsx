...
function Select({
  ...
  const handleOpenChange = useCallback(
    ...
    [cancelAckClose, commitOpenChange]
  );
  ...
  (
    ...
    return (
      <div className={cn("flex flex-col gap-1", wrapperClassName)}>
        ...
        {error && (
          <span className="text-label text-fg-danger pl-3">{error}</span>
        )}
      </div>
    );
  }
          ...
          >
            <SelectContentContext.Provider value={contentCtx}>
              <SelectPrimitive.Popup
                ...
                onMouseMove={(event) => {
                  popupProps.onMouseMove?.(event);
                  if (!event.defaultPrevented) handlers.onMouseMove(event);
                }}
                onMouseLeave={(event) => {
                  popupProps.onMouseLeave?.(event);
                  if (!event.defaultPrevented) handlers.onMouseLeave();
                }}
                onFocus={(e) => {
